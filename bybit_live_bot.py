...
ST_SIZE_PCT   = 0.38  # v6.2: 50→38% (12배 기준 최대 레버손실 -9.1%→-7%)
...
# ── 횡보 전략 파라미터 (v5.1 최적화) ──────────────
SW_SIZE_PCT   = 0.28  # v6.2: 35→28% (횡보 역추세는 더 보수적)
...
WEAK_SIZE_PCT = 0.22  # v6.2: 30→22% (약한추세는 확신 낮으므로 보수적)
...
ADX_SHORT_STRONG   = float(os.environ.get("ADX_SHORT_STRONG",  "28"))   # v6.2: 35→28 (숏 진입 가능하게 완화)
...
SHORT_SIZE_MULT     = float(os.environ.get("SHORT_SIZE_MULT", "0.85"))  # v6.2: 0.75→0.85 (숏 완화)
