...
[v6.3d 변경사항 - 2026-04-22 오후]
실전 1-2주 관찰 기반 타이밍 개선:
- 체제 전환 감지 느림 (30분~2시간 지연) → 고점 매수 → 손실 패턴
- "초반에 정확한 타이밍" 목표

1. STABILITY_N: 2 → 1 (체제 전환 즉시 반응)
2. EMA20 거리 필터 추가 (추세장 진입 시)
   - 가격이 EMA20 에서 EMA_DIST_MAX (1.5%) 이상 떨어지면
   - 즉시 진입 X, 눌림목 대기
3. 기존 눌림목 로직(PULLBACK_MODE)과 시너지
4. 최소 변경 원칙 (2개 포인트만) — 과적합 방지

[v6.3c 변경사항 - 2026-04-22 낮]
Bybit 30일 실전 데이터 기반:
...
STABILITY_N      = int(os.environ.get("STABILITY_N", "1"))            # v6.3d: 2→1 체제 전환 즉시 반응
...
# ── v6.3d: EMA 거리 필터 (추세장 진입 시 고점 매수 방지) ──
EMA_DIST_FILTER_ENABLED = os.environ.get("EMA_DIST_FILTER_ENABLED", "true").lower() == "true"
EMA_DIST_MAX            = float(os.environ.get("EMA_DIST_MAX", "0.015"))  # 1.5% 이상 떨어지면 대기
...
def run_loop():
    ...
    tg_plain(f"""🚀 바이비트 봇 v6.3d 시작 (타이밍 개선)
심볼: {', '.join(SYMBOLS)}
레버리지: {LEVERAGE}배
최대포지션: {MAX_POSITIONS}
포지션모드: {POSITION_MODE}
...
🆕 v6.3d 변경사항
- STABILITY 2→1 (즉시 반응)
- EMA20 거리 필터 {EMA_DIST_MAX*100:.1f}%
  - 추세장 진입 시 EMA에서 너무 멀면 대기
  - 고점 매수 방지 + 눌림목 진입
- 기존 v6.3c 구조 유지 (BTC only, 7x, SHORT 엄격)
━━━━━━━━━━━━━━
...
/stats /export /today /strategy""")
    ...
    # v6.3d: 명시적 루프 진입 로그 (봇 살아있음 확인용)
    print(f"[{now_kst().strftime('%H:%M:%S')}] ⚡ run_loop() 진입 — 메인 루프 시작", flush=True)
    tg_plain(f"⚡ v6.3d 메인 루프 진입 확인 ({now_kst().strftime('%H:%M')})")
...
ADX: {ind['adx']:.1f} | RSI: {ind['rsi']:.0f}""")
                ...
                # ══════════════════════════════════════════════════
                    ...
                    # v6.3d: EMA20 거리 필터 (고점 매수 방지)
                    if EMA_DIST_FILTER_ENABLED:
                        ema20_v = ind.get("ema20", 0)
                        if ema20_v > 0:
                            price_now_v = ind["price"]
                            ema_dist = abs(price_now_v - ema20_v) / ema20_v
                            # LONG: 가격이 EMA20 위로 1.5% 이상 떨어짐 → 눌림목 대기
                            if sig == "LONG" and price_now_v > ema20_v and ema_dist > EMA_DIST_MAX:
                                continue
                            # SHORT: 가격이 EMA20 아래로 1.5% 이상 떨어짐 → 반등 대기
                            if sig == "SHORT" and price_now_v < ema20_v and ema_dist > EMA_DIST_MAX:
                                continue
...
STABILITY: {STABILITY_N} | EMA 거리 필터: {EMA_DIST_MAX*100:.1f}% (ON={EMA_DIST_FILTER_ENABLED})
